...
extern float vertical_pixel_scale = 1.5;
...
// From 0 to 1.
extern float barrel_distort_x = 0.015;
...
extern bool pixel_bleed_enabled = false;
...
vec4 pixel_bleed(vec4 rgb, Image texture, vec2 pixel_coords)
{
	...
	if (square_pixels) {
		start_of_pixel_x = (float(int((pixel_coords.x * love_ScreenSize.x) / pixel_size)) - 0.3) * float(pixel_size) / float(love_ScreenSize.x);
		end_of_pixel_x = (float(int((pixel_coords.x * love_ScreenSize.x) / pixel_size)) + 0.3) * float(pixel_size) / float(love_ScreenSize.x);

		start_of_pixel_y = (float(int((pixel_coords.y * love_ScreenSize.y) / pixel_size)) + 0.3) * float(pixel_size) / float(love_ScreenSize.y);
		end_of_pixel_y = (float(int((pixel_coords.y * love_ScreenSize.y) / pixel_size)) - 0.3) * float(pixel_size) / float(love_ScreenSize.y);
	} 
    
	float mid_of_pixel_x = (start_of_pixel_x + end_of_pixel_x) / 2;
	float mid_of_pixel_y = (start_of_pixel_y + end_of_pixel_y) / 2;
...
extern float scanline_opacity = 0.2;
extern float scanline_center_fade = 0;

// How much space each line takes
extern float square_scanline_width = 0.1;
extern float horizontal_scanline_width = 0.1;
extern float vertical_scanline_width = 0.10;

vec4 desaturate(vec4 color, float amount)
{
    vec4 gray = vec4(dot(vec4(0.5126,0.7152,0.0722,0.3), color));
...
vec4 scanline_color(vec4 rgb, vec2 pixel_coords)
{
	...
	if (square_pixels) {
		...
		scanline_is_active = cos(current_pixel_v * 1.0 * PI) + square_scanline_width;
	...
	rgb_out = desaturate(rgb_out * 1.0, scanline_is_active * 0.16);
	...
	rgb_out.a = 0.85 - (scanline_is_active * scanline_opacity);

	return rgb_out;
}

const number offset_1 = 0.5;
const number offset_2 = 2.5;

const number alpha_0 = 0.15;
const number alpha_1 = 0.15;
const number alpha_2 = 0.15;
const number threshold = 5.0;
...
vec4 bloom(vec4 color, Image texture, vec2 texture_coords, vec2 pixel_coords)
{
   ...
   tc_v += Texel(texture, texture_coords + vec2(0.0, offset_1)/love_ScreenSize.y).rgb * alpha_1;
   tc_v += Texel(texture, texture_coords - vec2(0.0, offset_1)/love_ScreenSize.y).rgb * alpha_1;
   
   tc_v += Texel(texture, texture_coords + vec2(0.0, offset_2)/love_ScreenSize.y).rgb * alpha_2;
   tc_v += Texel(texture, texture_coords - vec2(0.0, offset_2)/love_ScreenSize.y).rgb * alpha_2;
   
   // Horizontal blur
   vec3 tc_h = texcolor.rgb * alpha_0;

   tc_h += Texel(texture, texture_coords + vec2(offset_1, 0.0)/love_ScreenSize.x).rgb * alpha_1;
   tc_h += Texel(texture, texture_coords - vec2(offset_1, 0.0)/love_ScreenSize.x).rgb * alpha_1;
   
   tc_h += Texel(texture, texture_coords + vec2(offset_2, 0.0)/love_ScreenSize.x).rgb * alpha_2;
   tc_h += Texel(texture, texture_coords - vec2(offset_2, 0.0)/love_ScreenSize.x).rgb * alpha_2;
...
vec4 effect(vec4 vcolor, Image texture, vec2 texture_coords, vec2 pixel_coords)
{
    ...
    working_rgb = bloom(working_rgb, texture, working_coords, working_coords);
	// scanlines
	if (scanline_enabled) {
